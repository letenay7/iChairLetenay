...
{
    private static readonly byte[] lookupHi = new byte[] {
    ...
    private static readonly byte[] lookupLo = new byte[] {
    ...
    public static ushort Checksum(byte[] bytes)
    {
        byte crcHi = 0xFF;
        byte crcLo = 0xFF;
        ...
        foreach (byte b in bytes)
        {
            ...
            crcLo = (byte)((crcHi ^ lookupHi[index]) & 0xFF);
            crcHi = lookupLo[index];
        }

        return (ushort)((crcHi << 8) | crcLo);
    }

    public static byte[] AddCrcBytes(byte[] bytes)
    {
        ushort crc = Checksum(bytes);
        byte[] withCrc = new byte[bytes.Length + 2];
        Array.Copy(bytes, withCrc, bytes.Length);
        withCrc[bytes.Length] = GetLSB(crc);
        withCrc[bytes.Length + 1] = GetMSB(crc);
        return withCrc;
    }

    public static byte GetLSB(ushort val)
    {
        return (byte)(val & 0xFF);
    }

    public static byte GetMSB(ushort val)
    {
